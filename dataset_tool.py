...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            print("Processing {}".format(image_filenames[order[idx]]))
            img = np.asarray(PIL.Image.open(image_filenames[order[idx]]))
            if channels == 1:
...
def read_uint16_png(filepath):
    with open(filepath, 'rb') as f:
        ...
        pilimg =  PIL.Image.fromarray(normalize0255(pixels_float))
        return pilimg
...
def create_from_images16(tfrecord_dir, image_dir, shuffle):
    ...
    #img = np.asarray(PIL.Image.open(image_filenames[0]))
    img = np.asarray(read_uint16_png(image_filenames[0]))
    resolution = img.shape[0]
    channels = img.shape[2] if img.ndim == 3 else 1
    if img.shape[1] != resolution:
        error('Input images must have the same width and height')
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(len(image_filenames))
        for idx in range(order.size):
            img = np.asarray(read_uint16_png(image_filenames[order[idx]]))
            if channels == 1:
                img = img[np.newaxis, :, :] # HW => CHW
            else:
                img = img.transpose(2, 0, 1) # HWC => CHW
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_from_images16', 'Create dataset from a directory full of images.',
                                            'create_from_images datasets/mydataset myimagedir')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_dir',        help='Directory containing the images')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
    ...
    #example:
    #python dataset_tool.py create_from_images  "gnorm_32_tf" "datasets/gnorm_32"
